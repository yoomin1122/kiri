...
class Admin(commands.Cog):
    ...
    @commands.has_permissions(administrator=True)
    @cog_ext.cog_slash(name="청소", description="청소를 할수 있습니다", 
    ...
    async def clean(self, ctx, number: int = None):
        if ctx.guild:   
                    if number is None:
                        ...
                        await ctx.message.delete()
                        await ctx.send(f'{ctx.author.mention} 100보다 큰 수는 입력할 수 없습니다.', delete_after=5)
                    else:
                        deleted = await ctx.channel.purge(limit=number)
                        embed = discord.Embed(title="청소 완료",description=f"청소된 메세지 : {len(deleted)}개",  color = 0xfa4f4f)
                        embed.set_footer(text=f"청소한 관리자 : {self.bot.author.name}")
                        await ctx.send(embed=embed)
        else:   
            await ctx.reply('DM에선 불가합니다.')
    ...
    options=[
        ...
        create_option(name="reason", description="사유를 적어주세요", required=True, option_type=3)])
    ...
    async def 밴(self, ctx, member: discord.Member, *, reason=None):
        if ctx.guild:
                ...
                await ctx.send(f'유저 {member}을(를) 밴하였습니다. \n사유 : {reason}')
        else:
            await ctx.reply('DM에선 불가합니다.')
